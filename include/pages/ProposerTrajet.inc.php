...
if (!empty($_POST["vil_num1"]) && empty($_POST["vil_num2"]) && empty($_POST["pro_date"]) && empty($_POST["pro_time"]) && empty($_POST["pro_place"])) {
    ...
    ?>
<form action="index.php?page=9" method="post">
    <p>Ville de départ : <?php echo $villeManager->getVilNom($_SESSION["vil_num1"]) ?></p>
    <label for="vil_num2">Ville d'arrivée : </label>
    <select name="vil_num2" id="vil_num2">
        <?php
        foreach ($parcoursManager->getVillesArrivees($_SESSION["vil_num1"]) as $ville) {
            ?>
            <option value="<?php echo $ville->getVilNum() ?>"><?php echo $ville->getVilNom() ?></option>
            <?php
        }
    ...
    <input type="date" name="pro_date" id="pro_date" value="<?php echo date('Y-m-d'); ?>">
    <label for="pro_time">Heure de départ : </label>
    <input  type="time" name="pro_time" id="pro_time" value="<?php echo date("H:i:s"); ?>" >
    <label for="pro_place">Nombre de places : </label>
    <input type="text" name="pro_place" id="pro_place">
    <input type="submit" name="submit" value="Valider">
</form>
<?php
}
